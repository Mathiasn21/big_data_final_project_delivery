...
val maks_atten2 = df_max.filter($"show".like("%18%")).groupBy("show").agg(max("count")).take(1)
val maks_all2 = df_max.filter($"show".like("%all%")).groupBy("show").agg(max("count")).take(1)*/
